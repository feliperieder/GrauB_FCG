...
const GLuint WIDTH = 1100, HEIGHT = 600;
...
enum sprite_states{MOVING_RIGHT=1, MOVING_LEFT, IDLE};
...
int main()
{
	...
	//-------------------------------------
	//Inicializando o Array das teclas
	...
	texID = loadTexture("../Textures/Characters/Boy.png",imgWidth,imgHeight);
	character = initializeSprite(texID, vec3(imgWidth*4,imgHeight*4,1.0),vec3(550,100,0),3,6, 4);

	texID = loadTexture("../Textures/Backgrounds/5.png",imgWidth,imgHeight);
	background = initializeSprite(texID, vec3(imgWidth*2,imgHeight*2,1.0),vec3(550,300,0));

	itemTexIDs[0] = loadTexture("../Textures/Items/Pear.png",imgWidth,imgHeight);
	itemTexIDs[1] = loadTexture("../Textures/Items/Orange.png",imgWidth,imgHeight);
	itemTexIDs[2] = loadTexture("../Textures/Items/Apple_Red.png",imgWidth,imgHeight);
	...
	mat4 projection = ortho(0.0, 1100.0, 0.0, 600.0, -1.0, 1.0);
...
void moveSprite(GLuint shaderID, Sprite &sprite)
{
	...
	if (!keys[GLFW_KEY_D] && !keys[GLFW_KEY_RIGHT] && !keys[GLFW_KEY_A] && !keys[GLFW_KEY_LEFT]){
...
void spawnItem(Sprite &sprite){
	...
	int max = lastSpawnX + 400;
	if (max > 790) max = 790;
	int min = lastSpawnX - 400;
	if (min < 20) min = 20;

	sprite.pos.x = rand()% (max-min+1) + min; //Valor entre 20 e 1080
	lastSpawnX = sprite.pos.x;
	sprite.pos.y = rand() % (2000 -650 +1) + 650; //Valor entre 650 e 850
	sprite.texID = itemTexIDs[rand()%3];
	sprite.vel = velItems;
	int n = rand()% 3 ;
	if (n ==1){
